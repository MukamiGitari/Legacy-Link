...
import { ArrowLeft, RotateCcw, CheckCircle2, XCircle, Sparkles, Quote } from 'lucide-react';
import { useApp } from '../../context/AppContext';
import { buildWhoSaidItRound, type WhoSaidItQuestion } from '../../lib/games';
...
export const WhoSaidIt: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const start = () => {
    const round = buildWhoSaidItRound(data, ROUND_SIZE);
    setQuestions(round);
    setQIndex(0);
    setScore(0);
    setSelected(null);
  ...
  const notEnoughData = buildWhoSaidItRound(data, 1).length === 0;
    ...
    <div className="space-y-6">
      ...
      {stage === 'menu' && (
        <div className="rounded-xl border border-heritage-cream-400 dark:border-heritage-dark-border bg-white dark:bg-heritage-dark-card p-8 max-w-lg mx-auto text-center">
          <Quote size={28} className="text-heritage-gold-500 mx-auto mb-3" />
          <p className="font-serif text-2xl text-heritage-green-900 dark:text-heritage-dark-text mb-2">Who Said It</p>
          <p className="text-sm text-heritage-green-600 dark:text-heritage-dark-muted mb-6">
            A family saying will appear — match it to the relative it's attributed to in your Language &amp; Sayings dictionary.
          </p>
          {notEnoughData ? (
            <p className="text-sm text-heritage-bark-600 italic">
              Add a few "Family Saying" entries in the Dictionary page (with who said it) to unlock this game.
            </p>
          ) : (
            <button
              onClick={start}
      ...
      {stage === 'playing' && current && (
        <div className="rounded-xl border border-heritage-cream-400 dark:border-heritage-dark-border bg-white dark:bg-heritage-dark-card p-6 max-w-xl mx-auto">
          ...
          <blockquote className="font-serif text-xl italic text-heritage-green-900 dark:text-heritage-dark-text mb-2 border-l-2 border-heritage-gold-400 pl-4">
            {current.saying}
          </blockquote>
          <p className="text-xs text-heritage-green-500 dark:text-heritage-dark-muted mb-5 pl-4">
            {current.meaning}
          </p>
          ...
          {selected !== null && (
            <div className="flex justify-end mt-5">
              ...
              >
                {qIndex + 1 >= questions.length ? 'See results' : 'Next saying'}
              </button>
            </div>
          )}
        </div>
